...
static func internal_get_motion_canonical_unsafe() -> Node:
...
static func internal_get_motion_canonical() -> Node:
	var canonical := internal_get_motion_canonical_unsafe()
		...
		if is_instance_valid(preset):
			if preset.name.is_empty():
				push_warning(internal_get_motion_canonical()
					.translate(&"ERROR_EMPTY_PRESET_NAME")
					.format([preset_index, name]))
				continue
...
func _enter_tree() -> void:
	var canonical := internal_get_motion_canonical_unsafe()
	if canonical != null:
		map_presets(canonical
			.get_preset_mapper())

func _exit_tree() -> void:
	var canonical := internal_get_motion_canonical_unsafe()
	if canonical != null:
		var preset_mapper: XDUT_MotionPresetMapper = canonical \
			.get_preset_mapper()
